    ...
    (() => {
        ...
        function menuInit() {
            if (document.querySelector(".icon-menu")) document.addEventListener("click", (function(e) {
                ...
                } else if (e.target.closest(".menu__close")) {
                    bodyUnlock();
        ...
        if (showContactsBtns) showContactsBtns.forEach((showContactsBtn => {
            showContactsBtn.addEventListener("click", (() => {
                console.log(showContactsBtn.parentNode);
                showContactsBtn.parentNode.classList.add("show-contact-active");
            }));
        }));
        const quickJumpBtns = document.querySelectorAll(".quick-jump__button");
        if (quickJumpBtns.length) {
            const searchInput = document.querySelector(".hero__input");
            for (const quickJumpBtn of quickJumpBtns) quickJumpBtn.addEventListener("click", (event => {
                searchInput.value = quickJumpBtn.children[1].textContent;
            }));
        }
        window["FLS"] = false;
        isWebp();
